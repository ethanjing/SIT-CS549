...
public class ControllerClient extends Endpoint implements MessageHandler.Whole<String> {
	...
	private final ClientEndpointConfig cec = ClientEndpointConfig.Builder.create().encoders(Arrays.asList(CommandLineEncoder.class)).build();  
	...
	public void onMessage(String message) {
		if (initializing) {
			if (SessionManager.ACK.equals(message)) {
				 ...
				 */
				try {
					shell.msgln("Control request was accepted");
				} catch (IOException e) {
					e.printStackTrace();
				}
